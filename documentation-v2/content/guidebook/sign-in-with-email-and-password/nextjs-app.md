...
Create `app/signup/page.tsx`. It will have a form with inputs for email and password.
...
In `app/signup/page.tsx`, redirect authenticated users to the profile page if their email is verified, or to the confirmation page if not.
...
Create `app/login/page.tsx`. It will have a form with inputs for email and password.
...
Create `routes/login/+page.server.ts` and handle POST requests.
...
In `app/login/page.tsx`, implement redirects as we did in the sign up page.
...
Return back to `app/api/signup/route.ts` and send the verification link.
