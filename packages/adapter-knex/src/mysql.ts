...
export default class KnexMySQLAdapter extends BaseKnexAdapter implements Adapter {
	public async getSessionAndUser(
		...
		const [result] = (await this.knex<KnexSessionTable>(this.tables.sessions)
			.select("*")
			.innerJoin<KnexUserTable>(
				this.tables.users,
				`${this.tables.sessions}.userId`,
				"=",
				`${this.tables.users}.userId`
			)
			.where(this.knex.ref(`${this.tables.sessions}.sessionId`), "=", sessionId)
			.options({ nestTables: true })) as unknown[] as {
			user: KnexUserTable;
			session: KnexSessionTable;
		}[];
