	...
	return {
		getUserByRefreshToken: async (refreshToken: string) => {
			try {
				...
				const dbUser = transformUserDocument(userDoc.docs[0]) as DatabaseUser;
				return dbUser;
			} catch (error) {
				console.error(error);
		...
		getUserByIdentifierToken: async (identifierToken: string) => {
			try {
				...
				const dbUser = transformUserDocument(user.docs[0]) as DatabaseUser;
				return dbUser;
			} catch (error) {
				console.error(error);
				throw new Error("DATABASE_FETCH_FAILED");
		...
		updateUser: async (userId: string, newData: any) => {
			...
			try {
				...
				userDoc = (await getDocumentFromDb("user", userQuery, couch)).docs[0] as DatabaseUser;
				const newDoc = {
					...userDoc,
					...partialData
				};
