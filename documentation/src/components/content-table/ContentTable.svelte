<script lang="ts">
	...
	export let selectedTab: typeof tabs[number]["tab"];

	const selectTab = (t: typeof selectedTab) => {
		selectedTab = t;
	};

	export let learnSections: Section[];
	export let referenceSections: Section[];
...
>
	<div class="overflow-auto h-full relative overscroll-contain pr-4">
		<div class="grid grid-cols-2 gap-x-1 sticky top-0 w-full bg-white dark:bg-black-zinc">
			<div class="w-full pb-1 border-b-2" class:border-main={selectedTab === "learn"}>
				<button
			...
			<div class="w-full pb-1 border-b-2" class:border-main={selectedTab === "reference"}>
				<button
					class="w-full mx-auto block text-center hover:bg-gray-100 dark:hover:bg-zinc-900 rounded py-1"
		...
		<div class="pt-4">
			{#each tabs as { tab: tabName, sections }}
